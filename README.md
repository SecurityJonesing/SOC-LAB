...
| C — Detection Engineering | ⏳ In progress | Atomic Red Team fully staged and working on Win11-LTSC-Victim. First custom detection rules (`100002`/`100003`, targeting PowerShell spawned by a suspicious parent process, escalating on encoded commands) written, debugged, and confirmed firing against real `T1059.001-17` atomic test traffic — three distinct root causes found and fixed (wrong `decoded_as`, `docker cp` file-ownership bug, wrong field-path namespace in the rule XML). Claude Code now has dedicated key-based SSH access to `wazuh-host`. SSH key-only hardening on `wazuh-host` complete (2026-08-11) — password auth disabled, key-based login verified. Remaining: rotate `wazuh-host` account password (was briefly typed in plaintext by an automation tool early on, caught before use); more atomic tests / rules to follow. |
