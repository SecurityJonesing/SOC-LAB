...
| C — Detection Engineering | ⏳ In progress | Atomic Red Team fully staged and working on Win11-LTSC-Victim. Custom detection rules `100002`/`100003` (PowerShell spawned by a suspicious parent process, escalating on encoded commands, T1059.001), `100004` (registry Run/RunOnce key persistence, T1547.001), and `100005` (LSASS credential dumping via Silent Process Exit / IFEO GlobalFlag abuse, T1003.001 + T1546.012) written, debugged, and confirmed firing against real atomic test traffic — three custom detections now spanning Execution, Persistence, and Credential Access. Claude Code now has dedicated key-based SSH access to `wazuh-host`. SSH key-only hardening on `wazuh-host` complete (2026-08-11) — password auth disabled, key-based login verified. Remaining: rotate `wazuh-host` account password (was briefly typed in plaintext by an automation tool early on, caught before use); more atomic tests / rules to follow. |
| C.5 — Network Visibility | ⏳ Not started | SPAN + Suricata |
| C.6 — Attack Surface & AD Expansion | ⏳ Not started | pfSense log forwarding, Kali isolation, WireGuard VPN, a full AD/DC build (`dc01`, `win11-ws02`, tiered OUs/groups/GPOs, six deliberate misconfigurations, file shares, ADCS), a Linux victim, expanded Kali tooling, and a full manual kill chain, reduced scope (no full interactive `dc01` compromise) |
