...
| C.5 — Network Visibility | ⏳ In progress | SPAN session configured on the switch (source VLAN 30, destination `Gi1/0/4`, ingress disabled), confirmed via `show monitor session 1`. Mirrored traffic path live end to end: switch → `pve01` NIC4/`eno4` → dedicated bridge `vmbr4` → `wazuh-host` second NIC (`ens19`), all interfaces up and promiscuous, no IPs assigned (monitoring-only path). Suricata 8.0.6 installed on `wazuh-host` (OISF PPA), bound to `ens19`, Emerging Threats Open ruleset loaded (52,713 rules enabled), service running clean, confirmed generating real alerts against live `nmap -sS` traffic from Kali. `/var/log/suricata` bind-mounted read-only into the Wazuh manager container; `<localfile>` block added to `ossec.conf` and Wazuh confirmed actively reading `eve.json` (`wazuh-logcollector: Analyzing file`). **Blocked:** Suricata alerts still absent from the dashboard — `wazuh-analysisd: ERROR: Too many fields for JSON decoder`, caused by Suricata's oversized `stats` events exceeding Wazuh's JSON decoder field limit. A Suricata-side fix (`enabled: no` on the eve-log `stats` type) was applied and verified saved/valid/restarted, but had no observed effect; cause unresolved. Next: establish current stats volume, then evaluate Wazuh-side filtering as the likely faster path. |
