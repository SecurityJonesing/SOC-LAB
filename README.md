...
| C — Detection Engineering | ⏳ In progress | Atomic Red Team fully staged and working on Win11-LTSC-Victim. Ran first atomic test (`T1059.001-17`) — Wazuh didn't alert. Root-caused through three layers (archive logging off, no archive index pattern, and the real cause: agent wasn't configured to read Sysmon's event channel at all) and fixed all three — Sysmon data now flows into Wazuh. Real example event captured (PowerShell via `cmd.exe`, Base64-encoded). Remaining: write and apply the first custom detection rule; rotate `wazuh-host` SSH password (was briefly typed in plaintext by an automation tool, caught before use) and set up a dedicated key. SSH key-only hardening on `wazuh-host` still scheduled alongside. |
