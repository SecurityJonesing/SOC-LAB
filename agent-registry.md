...
No agents exist yet, this will be populated for real starting in Phase 9.
...
- **Audit trail:** Every action logs as `agent=wazuh-triage-01`. My own
  ...
  convention to be finalized when Phase 9 is actually built, this is a
  placeholder label, not a name.)
- **Status:** Not built yet. Sequenced after Phase 7/8 in the current
  ...
  credential, least privilege, separate audit trail) before Phase 10 adds
  routing complexity on top.

### `triage-router-01` (Phase 10)
...
- **Credential:** TBD, decided when Phase 10 is actually built.
- **Scope:** TBD.
...
- **Status:** Not built yet. The underlying `pve-ai`/`ai-vm` GPU
  ...
  typing every command myself. This was a deliberate choice, not a
  shortcut around learning, I'd already done a Proxmox setup and an
  Ubuntu Server install once before this build, and had done GPU
  passthrough at work previously as well, so I already had hands-on
  familiarity with the concepts involved and delegated the repetitive
  execution to Claude Code while retaining review/confirmation authority
  over every step. The remaining Phase 10 work (Ollama, Open WebUI)
  reverts to this project's standard model instead, I run every command
  myself, for consistency with the rest of this build, since that part
  is new territory for me here.

### Shuffle playbook identity (Phase 11), new

- **Purpose:** Executes the Wazuh → Shuffle → enrichment → notify
  playbook. Registered here even before Phase 11 starts, since Shuffle
...
- **Status:** Not built yet. Sequenced after Phase 7, per the
...
### Entra Connect sync account (Phase 8), new
...
- **Scope:** Read access to synced attributes/objects only. No write-back
  beyond what the hybrid identity design in `LAB-BLUEPRINT.md` Phase 8
...
- **Audit trail:** Sync activity should be distinguishable in both AD's
  ...
  admin action.
- **Status:** Not built yet. Comes online as part of Phase 8's Entra
  Connect step.
