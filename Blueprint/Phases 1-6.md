...
### Phase 5 — Network Visibility (SPAN + Suricata) ✅ COMPLETE
...
7. ✅ **Persistence solved (2026-09-08):** the `tc` mirrors were runtime-only and did not survive a `pve01` reboot or a VM restart (which recreates the tap and silently drops that VM's mirror). Fixed with a Proxmox hookscript (`local:snippets/suricata-mirror-hook.sh`) attached to VMs 100, 101, and 103 — any of the three starting triggers a shared idempotent script that re-applies whichever mirror is missing. Enabling the `snippets` content type on Proxmox storage was required first (off by default). Caught and fixed a real bug during verification: the script's idempotency check (`grep`) was written against `tc`'s command syntax rather than its actual display output (`Egress Mirror to device <tap>`, not the typed `... dev <tap>` form), so it silently never matched and stacked a duplicate mirror filter on every VM start. Found via a missing `ADDED clsact qdisc` log line on a tap that hadn't restarted, fixed, and confirmed with a forced VM power cycle (`qm stop`/`qm start`, after `qm reboot` was found to be silently intercepted by Kali's XFCE desktop) producing the correct `OK ... already mirroring` result with exactly one filter per tap, no duplicates. Full writeup: `Build Logs/Phase 05 Network Visibility/Step 3 East West Gap and Mirroring/3.3 Hookscript Persistence and Duplicate Filter Fix.md`.

**Acceptance check status:** the original acceptance check (a network attack from Kali showing up in Wazuh from Suricata and having a host-based alert, two layers, one story) has been demonstrated live via the DHCP-hostname alert above. Persistence across reboots is now verified as well. Phase 5 is fully closed.
