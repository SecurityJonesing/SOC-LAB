...
- **`dc01` build in full** (VM creation, Windows Server install, `Install-WindowsFeature AD-Domain-Services`, `Install-ADDSForest` to create the `soclab.internal` forest) — I've done Windows Server installs and AD/DC standups before in real environments. No new learning value in typing this myself a second time.
...
- **Domain-joining both Win11-LTSC-Victim and `win11-ws02` to `soclab.internal`** — same reasoning as the `dc01` build: real prior hands-on experience with domain joins, nothing new to learn from typing it myself.
