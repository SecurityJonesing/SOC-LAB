...
Active job search, ~9 weeks of unemployment runway remaining. Lab work
is capped at roughly 8 hours total until employed:
1. Fix status drift in README.md and LAB-BLUEPRINT.md (Phase C shows as
   not-started; it's complete) — ~2 hrs
2. Write 3 investigation writeups for the completed detection rules — ~6 hrs

Everything else (C.5, C.6, C.7, D, E, F, G) is paused, not cancelled.
Do not propose build plans for paused phases unless Michael explicitly
reopens that decision. If asked "what should I work on," default to the
two items above only.
