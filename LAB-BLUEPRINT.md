...
2. **AD/DC build:**
   - Stand up `dc01` — Windows Server 2022, Core install, on RANGE30, sized modestly (2 vCPU / 4–8GB). **VM creation and Windows install are Claude Code-executed** (decided 2026-08-04) — real prior hands-on experience with Windows Server installs, no new learning value in typing it myself.
   - `Install-WindowsFeature AD-Domain-Services`, then `Install-ADDSForest` — new forest and domain, **`soclab.internal`** (not `.local`, to avoid mDNS conflicts; this is the forest root, not a join to an existing one). **Also Claude Code-executed** — real prior hands-on experience standing up AD/DC environments.
...
2. **Entra Connect (hybrid identity)** — build `entra-connect-01` on INFRA20, install Microsoft Entra Connect, sync `dc01` → Entra ID once Phase C.6's DC exists. Free-tier capable — full user/group/attribute sync plus one-way Password Hash Sync; only password *writeback* specifically needs P1. Requires the narrow INFRA20→RANGE30 rule described above, plus outbound rules for `entra-connect-01` to reach Microsoft's sync endpoints. Verify sync landed correctly and test SSO against a cloud resource. **Confirm the sync account's scope in `agent-registry.md`** — dedicated service account, minimum AD replication/read permissions, not a Domain Admin account — before the sync goes live, same governance discipline applied to every other non-human identity in this build.
3. **Exchange Online (Plan 1, $4/user/month)** — real mailboxes tied to the synced users. Phishing delivered via Defender for Office 365's **Attack Simulation Training** — Microsoft's own sanctioned tool, chosen specifically to avoid automated-abuse-detection ambiguity. Mail-flow/EOP logs become a genuine detection source.
...
**What moved to the Automated column (2026-08-06 revision, expanded 2026-08-04):** `dc01`'s full build (VM creation, Windows Server install, AD-DS role, forest creation — real prior hands-on experience, ~2–3 hrs), `win11-ws02`'s build (VM/Windows/Sysmon/Wazuh agent — repeats the proven Phase B pattern, ~2–3 hrs), domain-joining both Win11-LTSC-Victim and `win11-ws02` (~0.5–1 hr), AD account creation, OU placement, and group *membership* assignment (given prior real-world experience with exactly those tasks, ~1.5–2.5 hrs of mechanical execution, review time only — **not** group creation itself, which stays manual since I've only ever managed membership on existing groups), and the AD port-list firewall rules on RANGE30 (each repeats INFRA20's established rule shape, ~1–1.5 hrs). The OU/tiered-model *design*, group creation, GPOs, all six deliberate misconfigurations' exact configuration, the WireGuard VPN setup, and the INFRA20→RANGE30 Entra Connect rule remain fully manual — see "Execution model exceptions" in `PROJECT-INSTRUCTIONS.md` for the full reasoning.
