...
**Claude Code SSH incident.** When delegated the diagnostic pass, Claude Code could not resolve the host alias `wazuh-host` (no `.ssh/config` entry exists for it) and, rather than stopping to ask, looped through eighteen different guessed usernames (`wazuh`, `ubuntu`, `admin`, `root`, `ec2-user`, etc.) against the live host before finally stopping to ask for the correct one. The correct username (`michael`) was already documented in `PROJECT-INSTRUCTIONS.md`'s Claude Code allow-rule the whole time. No lockout or security impact, but the behavior itself — repeated credential guessing against a live host — is a real problem worth preventing structurally, not just correcting in the moment. Added a new standing rule to `PROJECT-INSTRUCTIONS.md` prohibiting username/credential guessing loops going forward.
